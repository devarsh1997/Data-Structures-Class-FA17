...
// double TileImage::getResize(int resolution) {
//     return static_cast<double>(getResolution()) / resolution;
// }
// void TileImage::paste(PNG& canvas, int startX, int startY, double ratio) const {
//     // If possible, avoid floating point comparisons. This helps ensure that
//     // students' photomosaic's are diff-able with solutions
//       for (int x = 0; x < resolution; x++) {
//           for (int y = 0; y < resolution; y++) {
//               double pixelStartX = (double)(x)     * ratio;
//               double pixelEndX   = (double)(x + 1) * ratio;
//               double pixelStartY = (double)(y)     * ratio;
//               double pixelEndY   = (double)(y + 1) * ratio;
//
    ...
    if (getResolution() % resolution == 0) {
        ...
        for (int x = 0; x < resolution; x++) {
            for (int y = 0; y < resolution; y++) {
                ...
                *(temp.getPixel(x,y)) = *(canvas.getPixel(startX + x, startY + y));
            }
        }
        image_ = temp;
    } else { // scaling is necessary
        double scalingRatio = static_cast<double>(getResolution()) / resolution;
        // PNG temp(int(image_.width()*scalingRatio), int(image_.height()*scalingRatio));
        PNG temp(resolution, resolution);
        for (int x = 0; x < resolution; x++) {
            for (int y = 0; y < resolution; y++) {
