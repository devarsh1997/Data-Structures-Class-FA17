...
class KDTree
{
  ...
  private:
    ...
    void sortKD(int left, int right, int dimension);
    Point<Dim> f(const Point<Dim>& query, int start, int end, int dimension) const;
    // Point<Dim> findNearestNeighbor(Point<Dim> target, Point<Dim> currBest, int lower, int upper, int dimension) const;
    //int calDist(const Point<Dim>& first, const Point<Dim>& second) const;
    // Point<Dim> findNearestNeighbor(int start, int end, const Point<Dim>& query, int dimension) const;
};
