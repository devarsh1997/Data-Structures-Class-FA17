                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    // TreeNode* smallest = new TreeNode(Node1->getFrequency()+Node2->getFrequency());
    // mergeQueue.push(smallest);
    /**
     * @todo Your code here!
     *
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned i=0; i<frequencies.size(); i++){
        ...
        TreeNode* leaf = new TreeNode(add);
        singleQueue.push(leaf);
    }

    while(singleQueue.size()+mergeQueue.size() > 1){
            // cout << "reach" <<endl;
        TreeNode* mergeNode1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* mergeNode2 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* mergeNode = new TreeNode(mergeNode1->freq.getFrequency()+mergeNode2->freq.getFrequency());
        mergeNode->left = mergeNode1;
        mergeNode->right = mergeNode2;
...
{
    TreeNode* current = this->root_;
    int next_bit;//might need use bool
    while (bfile.hasBits()) {
        next_bit = bfile.getNextBit();
        ...
        }else{ cout << "binary file contain unrecognizable bit" << endl;}
...
{
    while (bfile.hasBits()) {
        int next = bfile.getNextBit();//might need use bool
        // if (bfile.getNextBit()){
        if (next == 1){
            // Frequency* leaf_freq = new Frequency(bfile.getNextByte(), 0);
            // TreeNode* leaf = new TreeNode(leaf_freq);
