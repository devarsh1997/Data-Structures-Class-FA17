...
bool BinaryTree<T>::isOrdered(const Node* subRoot) const{
    if (subRoot->left == NULL && subRoot->right == NULL)
        return true;
    if (subRoot->left == NULL){
        return (isOrdered(subRoot->right) && (subRoot->elem <= subRoot->right->elem));
    }
    if (subRoot->right == NULL){
        return (isOrdered(subRoot->left) && (subRoot->elem >= subRoot->left->elem));
    }
    ...
    ) const
{
    ...
    if(subRoot->left != NULL)
        addNode(paths, small, subRoot->left);
    if(subRoot->right != NULL)
        addNode(paths, small, subRoot->right);
    small.pop_back();
    return;
}
...
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int ct) const{
    if (subRoot == NULL)
        return 0;
    ct += 1;
    int out = ct + sumDistances(subRoot->left, ct) + sumDistances(subRoot->right, ct);
