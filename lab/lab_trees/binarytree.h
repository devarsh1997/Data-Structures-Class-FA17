...
class BinaryTree
{
  ...
  private:
    ...
    void addNode(vector<vector<T>> &paths, vector<T> small, Node* subRoot) const;
    int sumDistances(const Node* subRoot, int ct) const;
    // void addNode(vector<vector<T>> &paths, vector<T> small, typename BinaryTree<T>::Node* subRoot);
    // const BinaryTree<T>::
    /**
