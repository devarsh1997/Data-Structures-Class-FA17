...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
        string this_customer = ll.customer;
        string this_url = ll.url;
        time_t this_date = ll.date;

        string customer_url = this_customer+this_url;

        if (whenVisitedTable.keyExists(customer_url)){//update
            if (whenVisitedTable.find(customer_url) < this_date){
                whenVisitedTable[customer_url] = this_date;
            }
        }else{//insert
            whenVisitedTable.insert(customer_url, this_date);
        }
        if (!pageVisitedTable.keyExists(this_url)){
            uniqueURLs.push_back(this_url);
            pageVisitedTable[this_url] = true; 
        }      
    }
}
